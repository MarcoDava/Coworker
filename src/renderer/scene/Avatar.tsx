...
export function Avatar({ position, color, rotationY = 0, isIdle, isTyping, transparent = false, skinColor = '{skinColor}', hairColor = '#3a2010' }: Props) {
  ...
  useFrame(({ clock }) => {
    ...
    if (isTyping) {
      ...
      lh.position.z = THREE.MathUtils.lerp(lh.position.z, 0.52, 0.12);
      rh.position.z = THREE.MathUtils.lerp(rh.position.z, 0.52, 0.12);
    } else {
  ...
  return (
    <group position={position} rotation={[0, rotationY, 0]}>
      ...
      {/* Chair back — behind the character (toward desk) */}
      ...
      <mesh position={[-0.36, 0.90, 0.0]} rotation={[0, 0, 0.32]} castShadow>
